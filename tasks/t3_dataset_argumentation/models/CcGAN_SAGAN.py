...
class ConditionalBatchNorm2d(nn.Module):
    ...
    def forward(self, x, y):
        out = self.bn(x)
        gamma = self.embed_gamma(y).reshape(-1, self.num_features, 1, 1)
        beta = self.embed_beta(y).reshape(-1, self.num_features, 1, 1)
...
class CcGAN_SAGAN_Generator(nn.Module):
    ...
    def forward(self, z, labels):
        ...
        out = out.reshape(-1, self.gene_ch*16, 4, 4) # 4 x 4
...
class CcGAN_SAGAN_Discriminator(nn.Module):
    ...
    def forward(self, x, labels):
        ...
        out = out.reshape(-1, self.disc_ch*16*4*4)
