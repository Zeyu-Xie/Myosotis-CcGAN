...
class cGAN_concat_SAGAN_Generator(nn.Module):
    ...
    def forward(self, z, labels):
        # n x z_dim
        act0 = self.snlinear0(torch.cat((z, labels.reshape(-1,1)),dim=1))            # n x g_conv_dim*16*4*4
...
class cGAN_concat_SAGAN_Discriminator(nn.Module):
    ...
    def forward(self, x, labels):
        ...
        out = out.reshape(-1,self.d_conv_dim*16*4*4)
        output = self.snlinear1(torch.cat((out, labels.reshape(-1,1)), dim=1))
