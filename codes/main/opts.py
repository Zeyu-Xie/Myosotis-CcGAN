...
def parse_opts(args_list=None):
    ...
    if args_list is None and 'ipykernel' in sys.modules:
        args_list = []
...
def parse_opts_from_preset(preset_name):
    if preset_name not in args_json:
        raise ValueError(f"Preset '{preset_name}' not found in {JSON_PATH}")
    preset_args = args_json[preset_name]
    args = parse_opts(args_list=preset_args)
    return args