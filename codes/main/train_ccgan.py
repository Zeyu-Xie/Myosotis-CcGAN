...
from opts import parse_opts, parse_opts_from_preset
...
def train_ccgan(kernel_sigma, kappa, train_images, train_labels, netG, netD, net_y2h, save_images_folder, save_models_folder = None, save_outputs_folder=None, clip_label=False):
    
    '''
    Note that train_images are not normalized to [-1,1]
    '''

    loss_df = pd.DataFrame(columns=['niter', 'd_loss', 'g_loss', 'real_prob', 'fake_prob'])
    ...
    for niter in range(resume_niters, niters):
        ...
        for _ in range(num_D_steps):
            ...
            d_loss = torch.mean(real_weights.reshape(-1) * d_loss_real.reshape(-1)) + torch.mean(fake_weights.reshape(-1) * d_loss_fake.reshape(-1))
    ...
    loss_df.to_csv(save_models_folder + "/CcGAN_{}_{}_nDsteps_{}_loss_intrain.csv".format(gan_arch, threshold_type, num_D_steps), index=False)
...
def sample_ccgan_given_labels(netG, net_y2h, labels, batch_size = 500, to_numpy=True, denorm=True, verbose=True):
    ...
    with torch.no_grad():
        ...
        while n_img_got < nfake:
            ...
            y = torch.from_numpy(fake_labels[n_img_got:(n_img_got+batch_size)]).type(torch.float).reshape(-1,1).cuda()
